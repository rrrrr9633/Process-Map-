                ...
                "annotations": [
                    {
                        "annotation_id": "A001",
                        "label": "标注短名",
                        "region": {"page": 1, "x": 0.1, "y": 0.2, "width": 0.02, "height": 0.02, "unit": "ratio"},
                        "raw_text": "图纸原始标注内容",
                ...
                "text": json.dumps(
                    {
                        ...
                        "rules": [
                            "只返回 JSON，不要返回 Markdown、解释、图片或 HTML。",
                            ...
                            "每条 annotation 必须独立，不要把多个尺寸、多个技术要求合并成一条。",
                            "label 必须是短标签，最多 6 个中文字符，例如 主轴颈、连杆颈、圆角、粗糙度、动平衡、清洁度、磁探、材料。",
                            "parameter_name 必须是简洁中文工程名称；parameter_value 只放关键值，不要塞入长段说明。",
                            "raw_text 保存图纸原始标注内容；normalized_text 用显示安全写法：Ra 写 Ra，粗糙度图形符号省略，Ø/∅ 统一写 Φ，± 写 ±，× 写 ×，° 写 °。",
                            "不要输出容易渲染成方块的形位/粗糙度 Unicode 符号；形位公差符号用中文描述，例如 ⊥ 写 垂直度，∥ 写 平行度。",
                            "annotations 中 region 坐标必须使用 unit=ratio；若提供了 view_region 则相对当前视图裁剪图 0~1，否则相对整页 0~1。",
                            "region 不是标注文字框，也不是整条长线范围；region 必须表示气泡箭头应该指向的几何目标、引线末端、尺寸线端点、公差框目标或技术要求文字起点。",
                            "region 必须是小目标框，建议 width/height 在 0.008~0.035 之间；禁止输出横跨整张图的大框，禁止指向空白处、大面积剖面线或图纸中心。",
                            "如果标注内容清楚但坐标不确定，可以返回 annotation，但 review_status 必须是 needs_manual_review，review_reason 写明坐标需人工校正。",
                            "如果内容或坐标都无法判断，不要强行创建 annotation，只在 risk_notes 说明。",
